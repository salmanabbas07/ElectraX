...
// Test with 200 products
const products = buildProducts(200);
console.log('Sample products (first 10):');
products.slice(0, 10).forEach(p => {
  console.log(`ID: ${p.id}, Category: ${p.category}, Image: ${p.image}`);
});
...
console.log(`Duplicates: ${allImages.length - uniqueImages.size}`);
...
const duplicates = Object.keys(imageCount).filter(img => imageCount[img] > 1);
if (duplicates.length > 0) {
  console.log('\nDuplicate images:');
  duplicates.forEach(img => {
    console.log(`${img} (used ${imageCount[img]} times)`);
  });
}
...
products.forEach(p => {
  ...
  categories[p.category].add(p.image);
});

console.log('\nImages by category:');
Object.keys(categories).forEach(cat => {
  console.log(`${cat}: ${categories[cat].size} unique images`);
});
